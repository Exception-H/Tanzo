...
export function MessageTokenUsage({
  ...
  return (
    <div
      className={cn(
        'ml-1.5 flex h-3.5 items-center font-mono text-[0.625rem] tabular-nums lowercase text-muted-foreground/50',
        className
      )}
    >
      {visibleEntries.map((entry, i) => (
        <span key={entry.label} className="flex items-center">
          {i > 0 ? <span aria-hidden className="mx-2.5 h-2.5 w-px bg-border/50" /> : null}
