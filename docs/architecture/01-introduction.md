...
- `name: tanzo`，`main: ./out/main/index.js`，作者 Lumin Studio。
- 运行时核心依赖 `ai@7.0.0-beta.116` 与 `@ai-sdk/*` 一系列 beta provider 包；UI 用 React 19、`@tanstack/react-query`、`zustand`、`react-router-dom@7`；持久化用 `better-sqlite3`；检索用 `@vscode/ripgrep`；MCP 用 `@ai-sdk/mcp`。
...
对话里流动的一切都是一个 `TanzoUIMessage` 的 `part`：文本、推理、工具调用、文件 diff、计划、子代理进度、遥测，都是 part 的一种类型。类型定义见 `src/shared/agent-message.ts:411`：

```ts
export type TanzoUIMessage = UIMessage<TanzoMetadata, TanzoDataParts, TanzoToolUI>
```

`TanzoTools`（工具词汇，`agent-message.ts:38`）与 `TanzoDataParts`（数据 part 词汇，`agent-message.ts:240`）是两套开放联合，三进程共引。详见 [04 跨进程契约](./04-ipc-and-contracts.md)。

### 3.2 一道边界（Seam）

renderer ↔ main 的对话数据只经由 IPC 的 `chat:*` 通道流动，载荷的核心是 `InferUIMessageChunk<TanzoUIMessage>`（流式 chunk）。设置类控制面（`provider:*`、`policy:*`、`mcp:*` 等）是独立的 CRUD 面，不走对话缝。不开本地端口，不走 localhost-SSE。详见 [ADR-0004](./adr/0004-ipc-as-the-only-seam.md)。

### 3.3 内层循环用 ToolLoopAgent

main 端**不手写 turn loop**。每个回合用 ai-sdk 的 `ToolLoopAgent`（`buildAgent`，`src/main/agent/runtime/build-agent.ts:52`）跑「调模型 → 执行工具 → 回灌 → 再调」的多步循环。`service.run` 在其外面只包了一层**压缩 / 续航**的外层循环（上限 `MAX_CONTINUATION_PASSES = 10`）。详见 [10 Agent 运行时](./10-agent-runtime.md) 与 [ADR-0001](./adr/0001-use-ai-sdk-toolloopagent.md)。

### 3.4 审批活在消息里

工具审批不在 main 维护 pending 状态机。`toolApproval` 决策函数返回 `user-approval` 时该回合自然停止；用户响应被写进消息历史，下一次 `stream` 用含响应的完整历史重跑，SDK 看到已批准的调用直接执行。main 唯一**承载正确性**的跨调用状态是取消用的 `AbortController`。详见 [13 策略与审批](./13-policy-and-approval.md) 与 [ADR-0002](./adr/0002-stateless-approval-rerun.md)。

### 3.5 main 是唯一真源

消息只持久化在 main 的 SQLite（`messages` 表，存完整 `TanzoUIMessage[]` 的 JSON）。renderer 永不落盘——它在内存里用 `ChatSession` 重建当前对话，运行结束后从 main 重新拉取对齐。详见 [22 持久化](./22-persistence.md)、[30 渲染层](./30-renderer.md) 与 [ADR-0003](./adr/0003-main-as-source-of-truth.md)。
...
- [ ] main 没有手写 turn loop；`agent.stream` 是内层执行核心。
...
| **ToolLoopAgent** | ai-sdk 提供的多步工具循环 agent，Tanzo 的内层执行核心 |
| **Run / 运行** | 一次 `agent.stream` 驱动的对话回合（可能含多步） |
