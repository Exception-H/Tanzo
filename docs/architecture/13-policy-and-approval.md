...
工具审批是一个**集中决策点**而非分散在各工具里的逻辑。每次工具调用前，`ToolLoopAgent` 调 `toolApproval` 回调（`buildAgent` 注入，`runtime/build-agent.ts:64`）。`agent/module.ts` 会先用 hooks 包裹 `PolicyEngine.decide`：`PreToolUse` 钩子可拒绝，否则继续交给标准策略引擎返回 ai-sdk 的 `ToolApprovalStatus`：

```
'approved' | 'denied' | 'user-approval' | 'not-applicable'
```

返回 `user-approval` 时该回合**自然停止**、把审批请求 part 流回 renderer——此刻 main **不持有任何审批状态**。用户响应被写进消息历史，下一次 `submit` 用含响应的完整历史重跑，SDK 看到已批准的调用直接执行（[ADR-0002](./adr/0002-stateless-approval-rerun.md)）。

## 2. 决策函数 `decide`（`policy/engine.ts:163`）

`buildAgent` 从 `providerMetadata.tanzo` 取 `kind`/`fingerprintFields`，从 `runtimeContext` 取活跃模式与 chatId，传入 `decide`。`createAgentModule` 包裹后的实际求值顺序是：
...
4. **已记忆决策**（持久 `listDecisions` 或 session 缓存），按 `fingerprint(toolName, input, fingerprintFields)` 匹配。
...
**失败安全**：任何异常降级为 `user-approval` 并记日志（`engine.ts:199`）——规则求值 bug 永不会静默批准。
...
`mergeRules`（`engine.ts:112`）固定优先级：内置 deny → 用户 deny → 用户 allow → 内置 allow → 用户 ask。`PolicyStore` 持久化规则、决策、每对话模式。
...
PolicyEngine 在 toolApproval 返回 'user-approval'
  ...
  → main 用含审批响应的完整历史起全新 agent.stream
  → SDK 在 step 起点看到已批准调用，直接执行、继续
```

`applyApprovalResponses`（`approval-responses.ts:24`）是纯共享逻辑：遍历 assistant 工具 part，用 ai-sdk 的 `isToolUIPart`/`isDynamicToolUIPart`/`getToolName` 识别并迁移状态，返回新消息 + `AppliedApprovalResponse[]`。

子代理审批经 `SubagentApprovalBroker` 路由到 root 对话（[12 工具系统](./12-tools.md) §6）。
